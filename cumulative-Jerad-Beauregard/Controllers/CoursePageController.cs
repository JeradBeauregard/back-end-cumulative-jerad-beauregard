...
namespace cumulative_Jerad_Beauregard.Controllers
{
	public class CoursePageController : Controller
	{
		...
		public IActionResult UpdateCourse(int id, string coursecode, string teacherid, string startdate, string enddate, string coursename)
		{
			Course UpdatedCourse = new Course();

			UpdatedCourse.CourseCode = coursecode;
			UpdatedCourse.TeacherId = Convert.ToInt32(teacherid);
			UpdatedCourse.StartDate = DateOnly.FromDateTime(Convert.ToDateTime(startdate));
			UpdatedCourse.FinishDate = DateOnly.FromDateTime(Convert.ToDateTime(enddate));
			UpdatedCourse.CourseName = coursename;

			int CourseId = _api.UpdateCourse(id, UpdatedCourse);
